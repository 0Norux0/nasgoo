...
## Routes relevant to the 5 v10.4 defects
...
**v10.4 difference vs v10.2:** the Placeholder API calls are now valid Filament 3.x (`->extraAttributes` instead of removed `->disableLabel`). Same route + same controller + same VendorFileLinks helper.
...
**v10.4 difference vs v10.2:** the controller is unchanged (v10.1 `unset($data['images'])` preserved at lines 120 + 195). But `Product::fill()` now has a model-level override that strips `images` from ANY mass-assignment, covering paths beyond these two routes (Filament admin product create, factories, future code).
...
**v10.4 difference vs v10.2:** routes unchanged. The new `vendor-order-status-dropdown` on the Show page posts to these existing routes based on the selected option.

### Defect 5 — no new routes; CSS-only fix

## All v10.x routes (from v10.2 route report — unchanged in v10.4)
...
If any of the above routes are MISSING from the dev's `route:list` output, the routes file in deployment is NOT the v10.4 routes/web.php. Re-extract the archive.
